...
aws ec2 register-image --image-location $bucket/$TIMESTAMP/nixos.img.manifest.xml --name "$name $TIMESTAMP $(basename $p)" --region us-east-1 --virtualization-type hvm | tee image.json
